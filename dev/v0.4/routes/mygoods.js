...
router.post('/post', upload.single('goo_img'), (req, res) => {
	...
	const sql_create='INSERT INTO t1_goods (goo_id, goo_name, goo_type, goal_price, mem_id, regist_day, time_year, time_month, time_day, time_hour, time_minute, status, goo_img) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)';
	client.query(sql_create, [input_goods.goo_id, input_goods.goo_name, input_goods.goo_type, input_goods.goal_price, input_goods.mem_id, input_goods.regist_day, input_goods.time_year, input_goods.time_month, input_goods.time_day, input_goods.time_hour, input_goods.time_minute, input_goods.status, input_goods.goo_img], (error, results, fields) => {
	  if (error) {
	   	res.redirect('/');
		console.log(error)
	  } else {
